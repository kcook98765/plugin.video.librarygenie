...
def debug_imdb_info(params):
    ...
    try:
        ...
        try:
            media_items = db_manager.fetch_data('media_items', f"imdbnumber = '{imdb_id}'")
            if media_items:
        ...
        try:
            imdb_exports = db_manager.fetch_data('imdb_exports', f"imdb_id = '{imdb_id}'")
            if imdb_exports:
        ...
        debug_info.append("LISTS CONTAINING THIS IMDB:")
        try:
            ...
            if media_items:
                ...
                list_items = db_manager.fetch_data('list_items', f"media_item_id IN {media_ids_str}")
                if list_items:
                    list_ids = [str(item['list_id']) for item in list_items]
                    list_ids_str = "(" + ",".join(list_ids) + ")"
                    
                    lists = db_manager.fetch_data('lists', f"id IN {list_ids_str}")
        ...
        debug_info.append("KODI LIBRARY STATUS:")
        try:
            from resources.lib.jsonrpc_manager import JSONRPC
            jsonrpc = JSONRPC()
