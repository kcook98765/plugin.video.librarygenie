...
def show_library_status():
    ...
    try:
        ...
        # Get items with valid IMDb IDs (starting with 'tt')
        imdb_result = query_manager.execute_query(
            "SELECT COUNT(*) as count FROM media_items WHERE source = 'lib' AND imdbnumber LIKE 'tt%'",
            fetch_one=True
        )
