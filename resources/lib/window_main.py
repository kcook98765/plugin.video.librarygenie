...
class MenuWindow(BaseWindow):
    ...
    def show_lists(self):
        """Display lists browser with proper navigation"""
        try:
            # Create a dummy item_info for the list browser
            dummy_item_info = {
                'title': 'List Browser',
                'kodi_id': 0,
                'year': '',
            ...
            # Open the main window which shows the hierarchical list structure
            from resources.lib.window_main import MainWindow
            main_window = MainWindow(dummy_item_info, "Browse Lists")
            main_window.doModal()
            del main_window

        except Exception as e:
            utils.log(f"Error opening list browser: {str(e)}", "ERROR")
            self.show_notification("Error opening list browser", xbmcgui.NOTIFICATION_ERROR)
