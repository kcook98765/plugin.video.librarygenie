            ...
            if is_search_history:
                ...
                return [
                    self._create_action(
                        ...
                        label=L(30214),  # "Save As New List"
                ...
                return [
                    self._create_action(
                        ...
                        label=L(30216).replace('%s', short_name),  # "Merge Into %s"
                        handler=self._handle_merge_lists,
                        payload={"list_id": context.list_id}
                    ),
                    self._create_action(
                        action_id="rename_list",
                        label=L(30217).replace('%s', short_name),  # "Rename '%s'"
                        handler=self._handle_rename_list,
                        payload={"list_id": context.list_id}
                    ),
                    self._create_action(
                        action_id="move_list",
                        label=L(30223).replace('%s', short_name),  # "Move '%s' to Folder"
