            ...
            if is_reserved or is_file_folder:
                ...
                return [
                    ...
                    self._create_action(
                        ...
                        label=L(30221).replace('%s', folder_name),  # "Create New List in '%s'"
                        handler=self._handle_create_list,
                        payload={"folder_id": context.folder_id}
                    ),
                    self._create_action(
                        action_id="create_subfolder",
                        label=L(30222).replace('%s', folder_name),  # "Create New Subfolder in '%s'"
                    ...
                    self._create_action(
                        ...
                        label=L(30217).replace('%s', folder_name),  # "Rename '%s'"
                        handler=self._handle_rename_folder,
                        payload={"folder_id": context.folder_id}
                    ),
                    self._create_action(
                        action_id="move_folder", 
                        label=L(30223).replace('%s', folder_name),  # "Move '%s' to Folder"
                    ...
                    self._create_action(
                        ...
                        label=L(30220).replace('%s', folder_name),  # "Delete '%s'"
