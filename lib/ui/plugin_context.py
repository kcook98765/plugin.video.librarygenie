...
class PluginContext:
    ...
    def __init__(self):
        ...
        # Navigation context - defer breadcrumb generation to router to avoid DB access
        breadcrumb_start = time.time()
        self.breadcrumb_path = None  # Router will generate this when needed
    ...
    def query_manager(self):
        ...
        if self._query_manager is None:
            ...
            # Note: Don't auto-initialize here - let handlers initialize when needed
            # This prevents unnecessary DB init on cache hits
        return self._query_manager
    
    def get_initialized_query_manager(self):
        """Get query manager and ensure it's initialized"""
        qm = self.query_manager
        if qm and not qm.initialize():
    ...
    def display_folder(self, folder_id: Optional[str]):
        ...
        try:
            ...
            # Handle root folder case (None/NULL)
            if folder_id is None:
                folder_info = None
                folder_name = "Root Level"
                self.logger.debug("Displaying root level folder")
            else:
            ...
            # Get subfolders (convert None to proper parameter)
            subfolders = query_manager.get_all_folders(parent_id=int(folder_id) if folder_id is not None else None)
            ...
            menu_items.append({
                'label': f"{L(30515)} for '{folder_name}'",
