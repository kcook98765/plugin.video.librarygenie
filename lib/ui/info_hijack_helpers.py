...
def open_native_info_direct(db_type: str, db_id: int, logger, title: str | None = None,
    ...
    try:
        ...
        xbmc.sleep(100)  # Give close command time to process

        # Get actual movie data from database for better ListItem setup
        movie_data = None
        if db_type == "movie":
            data = jsonrpc("VideoLibrary.GetMovieDetails", {
                "movieid": int(db_id), 
                "properties": ["title", "file", "plot", "year", "genre", "director"]
            })
            movie_data = (data.get("result") or {}).get("moviedetails") or {}
            logger.debug(f"Retrieved movie data for {db_id}: title='{movie_data.get('title', 'N/A')}'")

        # Create ListItem with better metadata
        actual_title = title or (movie_data.get('title') if movie_data else f"{db_type} {db_id}")
        li = xbmcgui.ListItem(actual_title)
        
        # Set up the VideoInfoTag with comprehensive data
        ...
        # Add more metadata to help dialog recognition
        if movie_data:
            if movie_data.get('plot'):
                tag.setPlot(movie_data['plot'])
            if movie_data.get('year'):
                try:
                    tag.setYear(int(movie_data['year']))
                except (ValueError, TypeError):
                    pass
            # Set file path if available
            if movie_data.get('file'):
                tag.setPath(movie_data['file'])
            elif path_hint:
                tag.setPath(path_hint)
        elif path_hint:
            tag.setPath(path_hint)

        # Also set properties for additional compatibility
        li.setProperty('dbtype', db_type)
        li.setProperty('dbid', str(db_id))

        logger.debug(f"Opening Dialog().info for {db_type} {db_id} with title '{actual_title}'")
        t0 = time.perf_counter()
        xbmcgui.Dialog().info(li)

        # Wait for native info dialog to become active with more comprehensive checks
        end = t0 + max(0.5, timeout)  # Minimum 500ms wait
        cur = None
        last = None
        ...
        while time.perf_counter() < end:
            ...
            if cur in (12003, 10147, 12002, 12001):  # DialogVideoInfo variants
            ...
            if xbmc.getCondVisibility('Window.IsActive(DialogVideoInfo.xml)') or \
               xbmc.getCondVisibility('Window.IsActive(DialogVideoInfo)'):
                dialog_appeared = True
                logger.info(f"Dialog().info detected via condition for {db_type} {db_id} in "
                            f"{time.perf_counter() - t0:.3f}s")
                break
                
            # Check if we have modal dialog with video content
            if xbmc.getCondVisibility('System.HasModalDialog') and \
               xbmc.getInfoLabel('ListItem.Title') == actual_title:
                dialog_appeared = True
                logger.info(f"Dialog().info detected via modal+title match for {db_type} {db_id} in "
                            f"{time.perf_counter() - t0:.3f}s")
                break
            ...
            if cur != last:
                ...
                logger.debug(f"Waiting for videoinfo ({elapsed:.2f}s): dlg={cur}")
                last = cur
        ...
        if dialog_appeared:
            ...
            logger.debug(f"Final state: dialog_id={final_dialog_id}, has_modal={has_modal}")
            return False
