...
class SettingsManager:
    ...
    # Skin Control Settings
    GUI_TO_PRESET_MAP = {
        0: "auto",          # Auto-Detect
        1: "custom",        # Custom  
        2: "aeon_nox",      # Aeon Nox SiLVO
        3: "arctic_zephyr", # Arctic Zephyr Reloaded
        4: "confluence",    # Confluence 
        5: "estuary",       # Estuary (Default)
        6: "mimic"          # Mimic
    }
    
    PRESET_TO_GUI_MAP = {v: k for k, v in GUI_TO_PRESET_MAP.items()}
    
    SKIN_DETECTION_MAP = {
        "skin.estuary": "estuary",
        "skin.arctic.zephyr.mod": "arctic_zephyr", 
        "skin.aeon.nox.silvo": "aeon_nox",
        "skin.mimic": "mimic",
        "skin.confluence": "confluence"
    }

    def get_skin_control_preset(self) -> str:
        """Get skin control preset selection as string"""
        gui_index = self.addon.getSettingInt('skin_control_preset')
        return self.GUI_TO_PRESET_MAP.get(gui_index, "auto")

    def set_skin_control_preset(self, preset_key: str) -> None:
        """Set skin control preset selection"""  
        gui_index = self.PRESET_TO_GUI_MAP.get(preset_key, 0)
        self.addon.setSettingInt('skin_control_preset', gui_index)

    def get_down_control_list(self) -> str:
        """Get comma-separated list of Down navigation control IDs"""
        return self.addon.getSetting('down_control_list') or "50,55"

    def set_down_control_list(self, control_ids: str) -> None:
        """Set comma-separated list of Down navigation control IDs"""
        self.addon.setSetting('down_control_list', control_ids)

    def get_right_control_list(self) -> str:
        """Get comma-separated list of Right navigation control IDs"""
        return self.addon.getSetting('right_control_list') or "500,501,502"

    def set_right_control_list(self, control_ids: str) -> None:
        """Set comma-separated list of Right navigation control IDs"""
        self.addon.setSetting('right_control_list', control_ids)

    def get_parsed_control_lists(self) -> tuple[list[int], list[int]]:
        """Get parsed control lists as integer lists"""
        down_str = self.get_down_control_list()
        right_str = self.get_right_control_list()
        
        def parse_ids(id_string: str) -> list[int]:
            try:
                return [int(x.strip()) for x in id_string.split(',') if x.strip().isdigit()]
            except:
                return []
        
        return parse_ids(down_str), parse_ids(right_str)

    def get_skin_controls_configured(self) -> bool:
        """Check if skin controls have been configured (to avoid re-detecting)"""
        return self.addon.getSettingBool('skin_controls_configured')

    def set_skin_controls_configured(self, configured: bool) -> None:
        """Set flag that skin controls have been configured"""
        self.addon.setSettingBool('skin_controls_configured', configured)

    def detect_and_configure_skin_controls(self) -> bool:
        """Auto-detect current skin using database mappings"""
        try:
            import xbmc
            current_skin = xbmc.getInfoLabel("Skin.CurrentSkin").lower()
            
            self.logger.info(f"First run skin detection - Current skin: {current_skin}")
            
            # Use database to find mapping
            ...
            mapping = skin_manager.get_mapping_by_skin_id(current_skin)
            
            if mapping:
                # Known skin - apply mapping
                self.set_skin_control_preset(mapping['preset_key'])
                self.set_down_control_list(mapping['down_controls'])
                self.set_right_control_list(mapping['right_controls'])
                
                self.logger.info(f"Auto-configured for {mapping['display_name']}")
                return True
            else:
                # Unknown skin - use custom with defaults
                self.set_skin_control_preset('custom')
                self.set_down_control_list("50,55")
                self.set_right_control_list("500,501,502")
                
                self.logger.info(f"Unknown skin '{current_skin}' - using custom defaults")
                return False
                
        except Exception as e:
            self.logger.error(f"Error during skin detection: {e}")
